...
**B-roll selection is relevance-first.** Candidate clips are filtered and ranked against each shot's query using the provider's own per-clip metadata (Pixabay tags, the Pexels page slug, Coverr titles), so fuzzy-matched off-subject footage is dropped before download. Resolution never excludes a landscape clip — renditions below 720p are still accepted and merely demoted to the back of the relevance-ranked pool (alongside clips too short to fill a cut slot): a soft on-subject clip beats a sharp off-subject one. Shorts pull portrait-native footage (≥1280px tall) when available, falling back to center-cropped landscape otherwise.
