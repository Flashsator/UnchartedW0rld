...
- Pick one specific real subject that fits this sub-topic and obeys the subject STOCK-FOOTAGE RULE above (a common, widely-filmed creature — never an obscure unfilmable species).
...
- CONCRETE STAKES, NOT ABSTRACT WRAPPER: the subject and its single most surprising action/consequence must be present and prominent as ONE hard, specific detail the viewer can picture — a number, a body part, a named action, a place — carried by a strong verb or a high-stakes noun. The frame is the wrapper; the shock is the payload. Never let generic wrapper words ("what … actually does", "the truth about …", "an unbelievable secret") — OR a mystery-SOURCE opener that teases WHO found it instead of naming WHAT was found ("What a Baltimore Lab Found in …", "What Scientists Discovered When …", "What They Found When …") — OR a narrative-time or soft-question wrapper that delays the concrete action ("The Night a …", "Why a … ?", "When a … Finally …") — fill the title while the subject and the surprising thing stay vague or buried past the fold. A source (a lab, a place, "scientists", "they", "researchers") is never the hook; the creature and its shocking action are. Concrete (good): "The Termite That Explodes Itself to Kill Ants", "The Frog That Freezes Solid Every Winter and Wakes Up". Abstract, shock buried (weak): "What the Blue Crystals on This Termite's Back Actually Do", "The Frog With an Unbelievable Survival Secret", "What a Baltimore Lab Found in an Owl's 270° Neck Turn" (the source leads; the owl and the artery-rupture stake sit past the 40-char fold). Also weak — the dramatic-time or soft-question frame delays the verb: "The Night a Frog Froze Solid", "Why a Frog Can Survive Freezing"; prefer the declarative "The Frog That Freezes Solid Every Winter and Wakes Up".
- FRONT-LOAD THE SUBJECT: the concrete subject and a surprising verb/noun must appear within the first ~40 characters of the title — that is all a phone feed shows before truncation, and it is what search matches. Never bury the subject behind a label prefix ("Case File:", "Profile:", "Report:") that eats the opening characters before the topic appears. Likewise never let a SOURCE or place ("a Baltimore lab", "scientists", "researchers", "they") occupy those opening characters ahead of the subject — lead with the creature and its concrete stake ("An Owl Turns Its Head 270° Without Tearing an Artery"), not who studied it.
...
- Every section except the last ends on a hook into the next: an unresolved detail, a "but here is where it stops making sense", an unanswered question that the next section will pick up. EXCEPTION — for sections 0, 3 and 5 ONLY the OPENING ~45 seconds ship as a standalone Short (the audio is verbatim but TRUNCATED, not the whole section — see Shorts cut rule): a cold Short viewer never sees the next section OR the back half of this one, so these must resolve the question they opened INSIDE that opening window and only then carry a SOFT forward pull — never a bare cliffhanger that cuts off mid-curiosity.
...
- Sections 3 and 5 are republished as standalone Shorts, shown cold to viewers who have NOT seen any other part of the episode. CRITICAL — the Short is NOT the whole section: only the section's OPENING ships, about the first 45 seconds of speech (~120-140 words, the first 4-6 spoken sentences). Everything after that is heard ONLY in the long video. So the standalone Short equals that opening window, and everything a cold viewer needs — the hook AND its full answer — must live inside it. Never assume the back half of the section airs as the Short; it does not.
...
- LEAD WITH THE PHENOMENON, NOT THE MECHANISM (sections 3 and 5 — this is the single biggest reach lever): open on the astonishing thing a stranger can SEE and instantly picture — the visible behavior, the WHAT — or a sharp question about it. NEVER open on the internal anatomy, mechanism, or measurement that EXPLAINS it; that is the body's payoff, not the hook. Equally banned as an opener: the history/trivia angle — a famous scientist, a discovery year, who first studied it ("A sundew obsessed Darwin more than the origin of species" gives a stranger nothing to picture, and it underperformed its own episode's phenomenon-first teaser ~3x). Leading with the mechanism or the trivia reads as a dry textbook line and gets scrolled past in the first second, even when it names the subject. A real example: the Short that opened "An owl can spin its head almost all the way around — how?" got ~5x the views of its same-episode sibling that opened "An owl's neck bones are full of holes far too wide for the arteries running through them." Good (phenomenon/question first): "An owl turns its head nearly all the way around without tearing an artery — how?". Weak (mechanism first): "An owl's neck bones have extra hollow space cushioning the arteries." The section body THEN explains the mechanism and fully resolves it — so phenomenon-first hook and SELF-CONTAINED PAYOFF reinforce each other: question up front, answer in the body, clean and complete. AND RAISE THE STAKES: across ~80 published Shorts the openers that won were not merely visible, they carried DRAMA — predation or hunting, threat/defense/escape, deception or mimicry, a hidden weapon or a sense we don't have, or an "impossible body" that forces a "how?". A visible but low-stakes opener still gets scrolled ("a spider builds a perfect web", "a beetle snaps into the air" underperformed 3-5x). If the section's true content allows it, frame the hook around the highest-stakes true moment in it (what the subject DOES to survive, hunt, escape, or deceive) — never invent stakes the episode doesn't deliver.
...
- Section 0 is ALSO republished as the same-day teaser Short (it is the channel's highest-traffic Short, dropped beside the long video). Its spoken cold-open line is free to stay a subject-less mystery for the long-video viewer — do NOT change it — but that line reads flat as a standalone Short title, so section 0 carries a "shortsHook" too: a context-free, subject-named curiosity hook that teases the episode's central question without revealing the payoff. It need not match section 0's first spoken sentence verbatim; it supplies on the card the context the cold open withholds. Because this teaser is exactly where cold viewers leave the angriest "you never said the answer" comments, section 0 must ALSO land ONE small, complete, self-contained fact of its own before its forward hook — a genuine mini-payoff the teaser delivers standalone — while still WITHHOLDING the episode's single central reveal for the long video. Keep sentence 1's mystery and the sentence-2 promise tail exactly as the Opening rule specifies; the small resolved beat lands in the later sentences, and the shortsHook/card keeps a SOFT pull to the full story ("the full story is wilder"), not a hard cliffhanger.
- Give sections 0, 3 and 5 — and ONLY them — the "shortsHook" field described in the Shape. It is REQUIRED on all three; a missing shortsHook forces the Short to title off raw narration, which is weaker. Each shortsHook must obey the Shape's form: subject named in the first ~3 words, present tense, single spoken clause, ~8-14 words, 2nd-person stakes where honest, and NEVER an abstract/poetic noun-phrase or Title-Case label ("When Pollen Leaps the Gap" flopped — it is a chapter label, not a spoken hook).
- DIFFERENT FACETS (sections 3 and 5): these two Shorts publish on separate days from the SAME episode and compete for the same audience, so their shortsHooks must reveal DIFFERENT facets — pose different questions, name different behaviors or stakes. Do NOT restate the same headline claim twice with reworded words; if section 3's hook is "A wasp turns a caterpillar into a living food store", section 5's must open a genuinely different angle ("The caterpillar keeps eating while it is eaten alive"), not the same fact again. (Same-episode Shorts that share one headline cannibalize each other's views.) The same ban covers the rest of the episode's packaging: neither hook may restate the claim already made by the episode TITLE or by section 0's teaser shortsHook — a viewer who saw the long video's title or yesterday's teaser must meet a genuinely NEW hook, not the same claim reworded (a section-3 hook that echoed the episode title nearly word-for-word drew ~1/4 the views of its own teaser). Title, teaser hook, section-3 hook, section-5 hook: four shots at the same audience — spend them on four different facets (the TEASER alone may echo the title's central claim: that same-day pairing is deliberate, and the wider ban binds only sections 3 and 5).`;
}
