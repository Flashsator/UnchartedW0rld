...
- Every section except the last ends on a hook into the next: an unresolved detail, a "but here is where it stops making sense", an unanswered question that the next section will pick up. EXCEPTION — sections 0, 3 and 5 ship VERBATIM as standalone Shorts (see Shorts cut rule): a cold Short viewer never sees the next section, so these must FIRST resolve the question they opened and only then carry a SOFT forward pull — never a bare cliffhanger that cuts off mid-curiosity.
...
- SELF-CONTAINED PAYOFF (sections 3 and 5 — they ship VERBATIM as standalone Shorts to cold viewers): sentence 1 teases, but the section MUST then deliver and fully RESOLVE the specific question it opened — land a complete, satisfying answer a stranger grasps entirely on its own. NEVER let one of these sections just stop mid-curiosity: an unresolved Short earns "what was the answer / clickbait" complaints and kills the replays a Short lives on. Resolve only THIS section's own beat; do NOT give away the episode's single central reveal (that belongs to the template's reveal section) UNLESS this section IS that reveal section, in which case it pays the episode off in full. This requirement WINS over any per-section role above that says "end on a hook into the next": once the payoff has landed you may add at most ONE soft forward pull ("and that is not even the strangest part") — kept soft, never a bare cliffhanger.
